...
class ViewController: UIViewController {
    ...
    private lazy var numberTF: UITextField = MakerView.makerView.createTF(
        placeholder: "Enter phone number", backgroundColor: .white, cornerRadius: 10, leftView: true, rightView: true
    )
    ...
    @objc func loginButtonTapped(_ sender: UIButton) {
        if numberTF.hasText, let text = numberTF.text {
            if text.hasPrefix("996") && text.count == 12 {
                ...
                numberTF.text = ""
                numberTF.placeholder = "Номер должен содержать 996 и 12 символов!"
